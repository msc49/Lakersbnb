...
class Bookings
  ...
  def self.book(id:)
    if ENV['ENVIRONMENT'] == 'test'
      DatabaseConnection.setup(dbname: 'Lakersbnb_test')
    else
      DatabaseConnection.setup(dbname: 'Lakersbnb')
    end
  ...
  def self.check_dates(available_from:, available_to:)
    ENV['ENVIRONMENT'] == 'test' ? DatabaseConnection.setup(dbname: 'Lakersbnb_test') : DatabaseConnection.setup(dbname: 'Lakersbnb') 
